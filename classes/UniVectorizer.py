...
class Vectorizer:
    ...
    def __init__(self, v_type: str, **params):
        ...
        Parameters:
            v_type: (str): vectorizer type - (either tfidf, count or hash, that stand for
            TfidfVectorizer, CountVectorizer and HashingVectorizer from sklearn respectively)

            **params: (key-word args): parameters to be passed
        """

        if not v_type in {'tfidf', 'hash', 'count'}:
            raise Exception('This vectorizer is not supported')

        if v_type == 'tfidf':
            self.__vectorizer = TfidfVectorizer()
        elif v_type == 'hash':
            self.__vectorizer = HashingVectorizer()
        elif v_type == 'count':
    ...
    def get_feature_names(self) -> list:
        ...
        """
        if type(self.__vectorizer) is HashingVectorizer:
            raise Exception('This method is not supported for HashingVectorizer')
    ...
    def transform(self, X: pd.Series=None) -> pd.DataFrame:
        ...
        """
        if X is None:
            return self.__vectorizer.transform(self.data_ini)
